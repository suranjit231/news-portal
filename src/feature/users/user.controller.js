...
export default class UserController{
    ...
    //------ subscribe for get premium accesss controller
    async subscribe(req,res,next){
        try{
            let userId = req.userId;
            let {plan, duration} = req.body;

            duration = Number(duration);
            if(!plan || !duration || duration<1){
                return res.status(404).json({success:false, msg:"invalid subscription"});
            }

            const result = await this.userRepository.subscribeUser(userId, plan, duration);
            ...
            console.log("subscribe error controller: ", error);
            return res.status(404).json(error.msg)
        }
    }
    ...
    async getSubscription(req,res,next){
        try{
            const userId = req.userId;
            const mySubscription = await this.userRepository.getSubscription(userId);
            if(!mySubscription.success){
                return res.status(404).json(mySubscription.msg);
            }

            return res.status(404).json(mySubscription);

        }catch(error){
            console.log("get subscription user error: ", error);
            return res.status(404).json(error.msg)
        }
    }
